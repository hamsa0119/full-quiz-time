# 🚀 QuizTime Online Exam System - Quick Setup Guide

Welcome to the **QuizTime Online Exam System**! This guide will help you get the system up and running on a fresh computer in no time.
...
2.  Create a new database named `QuizTime_exam`:
    ```sql
    CREATE DATABASE QuizTime_exam;
    ```
3.  The backend is configured to use the following credentials by default:
    *   **Username:** `root`
    *   **Password:** *root*
    > [!TIP]
    > If your MySQL credentials are different, update them in `../QuizTime_Online_Exam_Backend/src/main/resources/application.properties`.
...
2.  Move into the backend directory (it's now a sibling to the frontend):
    ```bash
    cd ../QuizTime_Online_Exam_Backend
    ```
